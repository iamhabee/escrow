    ...
    >
      <div className={style.logoContainer}>
        <div className={style.logo}>
          <img src="resources/images/logo.svg" className="mr-2" alt="escrow" />
          <div className={style.name}>{logo}</div>
          {logo === 'Escrow' && <div className={style.descr}>ESC</div>}
        </div>
      </div>
      <div className={style.navigation}>
        <Menu onClick={handleClick} selectedKeys={selectedKeys} mode="horizontal">
          {generateMenuItems()}
          <Menu.Item>
            <button className="btn btn-outline-primary rounded-3" type="button" onClick={handleLogout}>Logout</button>
          </Menu.Item>
        </Menu>
      </div>
    </div>
